...
const isAllDigitsSame = (number) => {
  ...
  for (
    let head = 0, tail = numberString.length - 1;
    head < tail;
    ++head, --tail
  ) {
    if (
      numberCheckArray.includes(numberString.charAt(head)) &&
      numberCheckArray.includes(numberString.charAt(tail))
    ) {
      if (
        firstChar !== numberString.charAt(head) ||
        firstChar !== numberString.charAt(tail)
      )
        return false;
    } else {
      return false;
